...
.scan-button { @apply inline-flex items-center justify-center gap-2 rounded-lg bg-primary-700 px-3 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-primary-800 disabled:cursor-not-allowed disabled:opacity-50; }
.scan-overlay { @apply fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4; }.scan-dialog { @apply w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl; }.camera-preview { @apply relative flex aspect-video overflow-hidden rounded-xl bg-slate-900; }.camera-preview video { @apply h-full w-full object-cover; }.camera-placeholder { @apply absolute inset-0 flex flex-col items-center justify-center gap-2 text-sm text-slate-300; }
</style>
