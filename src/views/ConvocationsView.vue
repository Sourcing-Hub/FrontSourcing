...
<style scoped>
.convocation-toolbar { @apply mb-5 grid items-end gap-4 rounded-2xl bg-white p-4 ring-1 ring-slate-100 sm:grid-cols-2 lg:grid-cols-[190px_190px_auto_1fr]; }
...
.day-picker { @apply mb-5 w-full max-w-sm; }.day-picker label { @apply mb-1.5 block text-xs font-bold text-[#00313C]; }.day-picker select { @apply w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold capitalize text-[#00313C] outline-none transition focus:border-primary-500 focus:ring-2 focus:ring-primary-100; }
.convocation-workspace { @apply grid gap-5 lg:grid-cols-[minmax(280px,0.9fr)_minmax(380px,1.1fr)]; }.schedule-panel, .candidates-panel { @apply overflow-hidden rounded-2xl bg-white ring-1 ring-slate-100; }.panel-heading { @apply flex items-start justify-between border-b border-slate-100 px-5 py-4; }.panel-heading h3 { @apply text-sm font-bold text-[#00313C]; }.panel-heading p { @apply mt-0.5 text-xs capitalize text-slate-500; }.panel-heading > span { @apply rounded-full bg-primary-50 px-2 py-1 text-[10px] font-bold text-primary-700; }
...
.convocation-footer { @apply sticky bottom-3 z-10 mt-5 flex flex-col gap-3 rounded-2xl bg-white px-4 py-3 ring-1 ring-slate-100 sm:flex-row sm:items-center sm:justify-between; }.convocation-footer p { @apply text-xs text-slate-600; }.status-dot { @apply mr-1 inline-block h-2 w-2 rounded-full bg-emerald-500; }.convocation-footer > div { @apply flex flex-wrap justify-end gap-2; }.convocation-footer .btn-secondary, .convocation-footer .btn-primary, .send-button { @apply inline-flex items-center justify-center px-3 py-2 text-xs; }.send-button { @apply rounded-lg bg-pink-600 font-bold text-white transition hover:bg-pink-700 disabled:cursor-not-allowed disabled:opacity-50; }
