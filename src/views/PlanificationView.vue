...
const save = async () => {
  if (!editingId.value) {
    const { formation, cohorte, ...planningConfiguration } = configuration
        ...
        <div v-else class="flex-1 overflow-y-auto p-5 sm:p-6">
          ...
          <div class="mt-5 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <section v-if="selectedDay">
              <div class="mb-3 flex items-center justify-between"><p class="section-title mb-0">Créneaux horaires <span class="capitalize">({{ formatDayName(selectedDay.date) }} {{ formatDayNumber(selectedDay.date) }})</span></p><button type="button" class="text-sm font-bold text-pink-600 hover:text-pink-700" @click="addSlot(selectedDay)">+ Créneau</button></div>
              <div class="space-y-3">
