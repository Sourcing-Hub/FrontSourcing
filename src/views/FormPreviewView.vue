    ...
    <main class="max-w-3xl mx-auto mt-8 px-4 relative">
      ...
      <div v-else-if="formulaire" class="space-y-6">
        ...
        >
          ...
          <div>
            <template v-if="champ.type === 'TEXTE'">
              <input type="text" v-model="formResponses[champ.id]" :placeholder="champ.placeholderTexte || 'Votre réponse'" class="w-1/2 border-b-2 border-gray-300 hover:border-gray-400 focus:border-[#CE0033] focus:outline-none transition-colors py-1 text-sm bg-transparent" />
            </template>

            <template v-else-if="champ.type === 'ZONE_TEXTE'">
              <textarea v-model="formResponses[champ.id]" :placeholder="champ.placeholderTexte || 'Votre réponse'" rows="3" class="w-full border-b-2 border-gray-300 hover:border-gray-400 focus:border-[#CE0033] focus:outline-none transition-colors py-1 text-sm bg-transparent resize-y"></textarea>
            </template>
            ...
            <template v-else-if="champ.type === 'LISTE_DEROULANTE'">
              <select v-model="formResponses[champ.id]" class="w-64 border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[#CE0033] focus:border-[#CE0033] text-sm bg-white">
            ...
            <template v-else-if="champ.type === 'DATE'">
              <input type="date" v-model="formResponses[champ.id]" class="w-48 border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-2 focus:ring-[#CE0033] text-sm" />
            </template>
            
            <template v-else-if="champ.type === 'NOMBRE'">
              <input type="number" v-model="formResponses[champ.id]" :placeholder="champ.placeholderTexte || 'Votre réponse'" class="w-48 border-b-2 border-gray-300 hover:border-gray-400 focus:border-[#CE0033] focus:outline-none transition-colors py-1 text-sm bg-transparent" />
            </template>
              ...
              <div class="flex items-center justify-center w-full">
                <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                  <div class="flex flex-col items-center justify-center pt-5 pb-6">
                    <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">Cliquez pour ajouter un fichier</span> ou glissez-déposez</p>
                  </div>
        ...
        <div class="flex justify-between items-center py-4">
          <button type="button" class="bg-[#CE0033] hover:bg-[#a8002a] text-white font-medium py-2 px-6 rounded-md transition-colors shadow-sm">
            Envoyer
          </button>
